...
class MigrationTests(unittest.TestCase):
    ...
    def test_from_demog_and_param_gravity_webservice_population_only(self):
        ...
        #              [2, 2]]
        # for idx in range(n_nodes):
        #     nodes.append(Node(forced_id=idx + 1, pop=10000 if idx == 0 else 1000, lat=locations[idx][0],
        #                       lon=locations[idx][1]))
        #
        # demog = Demographics.Demographics(nodes=nodes, idref=id_ref)
        # demog.SetDefaultProperties()
        ...
        migration = from_demog_and_param_gravity_webservice(demographics_file, params="migration_parameters.json",
                                                            id_ref=id_ref,
                                                            migration_type=Migration.LOCAL)
        self.assertEqual(migration.NodeCount, 9)
        self.assertEqual(migration.DatavalueCount, 8)
        self.assertEqual(migration.IdReference, id_ref)
        self.assertEqual(migration.MigrationType, Migration.LOCAL)

        migration_file = CWD / 'data' / 'migration' / 'from_demog_and_param_gravity_webservice.bin'
        if migration_file.is_file():
            migration_file.unlink()
        migration.to_file(migration_file)
        ...
        for line in out[1:len(out)-1]:
            from_node, destination, rate = line.split(',')
            if from_node == '1' or destination == '1':
                high_rate.append(float(rate))
            else:
    ...
    def test_gravity_webservice_vs_local(self):
        ...
        migration_web = from_demog_and_param_gravity_webservice(demographics_file, params="migration_parameters_2.json",
                                                                id_ref=id_ref,
                                                                migration_type=Migration.REGIONAL)
        migration_local = from_demog_and_param_gravity(demographics_file, gravity_params=[0.1, 0.3, 0.6, -3],
                                                       id_ref=id_ref, migration_type=Migration.REGIONAL)

        # self.assertTrue(migration_web == migration_local)

        migration_local_file = CWD / 'data' / 'migration' / 'gravity_local.bin'
        migration_web_file = CWD / 'data' / 'migration' / 'gravity_webservice.bin'

        if migration_local_file.is_file():
            migration_local_file.unlink()
        if migration_web_file.is_file():
            migration_web_file.unlink()
        migration_web.to_file(migration_web_file)
        migration_local.to_file(migration_local_file)
    ...
    def test_gravity_webservice_vs_local_distance_only(self):
        ...
        migration_web = from_demog_and_param_gravity_webservice(demographics_file, params="migration_parameters_3.json",
                                                                id_ref=id_ref,
                                                                migration_type=Migration.REGIONAL)
        migration_local = from_demog_and_param_gravity(demographics_file, gravity_params=[1, 1, 1, -1],
                                                       id_ref=id_ref, migration_type=Migration.REGIONAL)

        migration_local_file = CWD / 'data' / 'migration' / 'gravity_local_distance_only.bin'
        migration_web_file = CWD / 'data' / 'migration' / 'gravity_webservice_distance_only.bin'
